 ...
 */
export const getAppraisalProps = state => ({
  cp: "", hp: "", sd: "",
  ...state.pokemon.appraisal || {}
})
 ...
 */
export const getIVComboSorter = state => [...state.pokemon.appraisalSorter || ["ivt", -1]]
...
const getAppraisalValues = createSelector(
  [getPokemon, getStardustOptions, getStardustLuckyMultiplier, getAppraisalProps],
    ...
    return cpMultipliers
      ...
      .reduce((list, { cpm, index }) => {
        ...
        for (let sta = min.sta; sta <= max.sta; sta++) {
          ...
          let combo =
            config.hp === hp ? { lv, cpm } :
            config.hp === hpStep ? { lv: lv + 0.5, cpm: cpmStep } :
            null
...
export const getIVCombinations = createSelector(
  ...
  (list, [sortKey, sortOrder]) => list
    .sort((c1, c2) => sortOrder * (c1[sortKey] - c2[sortKey]))
)
 ...
 */
function getIVTAppraisals(team, name) {
  switch (team) {
    case 0: return [
